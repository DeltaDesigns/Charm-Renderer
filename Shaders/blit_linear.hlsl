...
void PSMain(VSOutput input, out float4 rt : SV_Target0)
{
    float4 srgbColor = Source.Sample(Sampler, input.uv);
    rt.xyz = (srgbColor <= 0.04045) ? srgbColor / 12.92 : pow((srgbColor + 0.055) / 1.055, 2.4);
    //rt.w = srgbColor.w;
	rt.w = 1;
}
