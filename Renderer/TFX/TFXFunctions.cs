...
public static class TFXFunctions
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vec4 EvaluateLength4(Vec4 v)
    {
        float lenSq = Vec4.Dot(v, v);
        float rsqrtApprox = lenSq > 0f ? 1f / MathF.Sqrt(lenSq) : 0f;
        float refined = rsqrtApprox * (1.5f - 0.5f * lenSq * rsqrtApprox * rsqrtApprox);

        if (float.IsNaN(refined))
            refined = rsqrtApprox;

        float length = refined * lenSq;

        return new Vec4(length);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vec4 EvaluateNormalize4(Vec4 v)
    {
        float lenSq = Vec4.Dot(v, v);
        if (lenSq == 0f) return Vec4.Zero;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vec4 EvaluateNormalize3(Vec4 v)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vec4 EvaluateSmoothStep(Vec4 x, Vec4 edge0, Vec4 edge1)
    {
        static float SmoothStepComponent(float x, float edge0, float edge1)
        {
            float t = edge1 - edge0;
            t = MathF.Abs(t) > float.Epsilon ? (x - edge0) / t : 0f;
            t = Math.Clamp(t, 0f, 1f);
            return t * t * (3f - 2f * t);
        }

        return new Vec4(
            SmoothStepComponent(x.X, edge0.X, edge1.X),
            SmoothStepComponent(x.Y, edge0.Y, edge1.Y),
            SmoothStepComponent(x.Z, edge0.Z, edge1.Z),
            SmoothStepComponent(x.W, edge0.W, edge1.W)
        );
    }
}
